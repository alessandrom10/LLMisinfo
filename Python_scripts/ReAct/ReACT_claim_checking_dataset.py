...
with open("Results/claims_english_answers_after_70b.txt", "r", encoding="utf-8") as file:
...
claims, results = load_claims("Datasets/english_150_after_2024_sample.csv")
...
for m in range(len(claims)):
    ...
    for i in range(n_shots):
        ...
        if len(text) + len(pre) > 30000:
            text = text[len(text) + len(pre) - 30000:]
    ...
    pre = " Final Answer, choose between the options false, mostly false, mixture, mostly true and true: "  
    # pre_tok = tokenizer.encode(pre, add_special_tokens=False, return_tensors="pt").to("cuda")
    ...
    # add answer to file
    with open("Results/claims_english_answers_after_70b.txt", "a", encoding="utf-8") as file:
    ...
    with open("Results/PROMPTS_claims_english_answers_after_70b.pkl", "rb") as file:
        okey = pickle.load(file)
        okey[checked + m] = final_text
    with open("Results/PROMPTS_claims_english_answers_after_70b.pkl", "wb") as file:

...
with open("Results/claims_es_answers_70b.txt", "r", encoding="utf-8") as file:
...
for n in range(n_shots):
    with open(f"Prompts/fact_checking_react_{n+1}_es.txt", "r", encoding='utf-8') as file:
        react_shots += file.read()

# question to be answered
# text = react_shots + "<|user|> Question: Which is the strongest animal? <|end|>"
claims, results = load_claims("Datasets/spanish_150.csv")
...
for m in range(len(claims)):
    ...
    text = react_shots + "Afirmación 1: " + claim + ". Puedes elegir entre las opciones false, mostly false, mixture, mostly true and true. \n\n"
    ...
    for i in range(n_shots):
        ...
        pre = "Acción " + str(i + 1) + ": Buscar["
        # pre_tok = tokenizer.encode(pre, add_special_tokens=False, return_tensors="pt").to("cuda")

        generated_text = ("Acción "+str(i+1)).join(generated_text[0]["generated_text"].split("Acción "+str(i+1))[:max_iters +1])
        ...
        search = generated_text.split("Buscar[")[-1].split("]")[0]
        ...
        while search_info == "":
            try:
                ...
                print("Error in search, trying again")
                time.sleep(10)


        # cure search info

        text =  "Buscar[".join(generated_text.split("Buscar[")[:max_iters * n_shots + 1 + i]) + "Buscar[" + search + "] \n\n Resultado " + str(i+1) + ":" + search_info + "\n"
    ...
    pre = " Respuesta final, elige entre las opciones false, mostly false, mixture, mostly true y true: "

    # pre_tok = tokenizer.encode(pre, add_special_tokens=False, return_tensors="pt").to("cuda")


    if len(text) + len(pre) > 24000:
        text = text[len(text) + len(pre) - 24000:]
    ...
    while type(final_text) is type(dict()):
        ...
        if type(generated_text) is type(dict()) or "502" in final_text:
            # sleep for 10 minutes
            print(generated_text)
    ...
    # add answer to file
    with open("Results/claims_es_answers_70b.txt", "a", encoding="utf-8") as file:
    ...
    with open("Results/PROMPTS_claims_es_answers_70b.pkl", "rb") as file:
        okey = pickle.load(file)
        okey[checked + m] = final_text
    with open("Results/PROMPTS_claims_es_answers_70b.pkl", "wb") as file:
